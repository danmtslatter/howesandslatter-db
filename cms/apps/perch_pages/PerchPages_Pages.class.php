...
class PerchPages_Pages extends PerchAPI_Factory
{
    ...
    public function old_auto_reorder($new_only=true)
    {
    ...
    public function auto_reorder($new_only=true)
    {
        ...
        $max_depth = 0;
        $needs_to_rerun = false;

        if (PerchUtil::count($pages)) {
            foreach($pages as $Page) {
                $data = array();
                $data['pageDepth'] = (int)$this->_page_depth($Page->pagePath());
                $data['pageSection'] = $this->_page_section($Page->pagePath());
                $Page->update($data);
                
                if ($data['pageDepth']>$max_depth) $max_depth = $data['pageDepth'];
            }
        }
        ...
        if (!$new_only) {
            // reset orders
            $sql = 'UPDATE '.$this->table.' SET pageOrder="" ';
            $this->db->execute($sql);
        }
        ...
        for ($i=0; $i<=$max_depth; $i++) {
            ...
            if ($i==0 || $i ==1){
                $where = "pageDepth='0' OR pageDepth='1'";
            }else{
                $where = 'pageDepth='.$this->db->pdb($i);
            }
                    
            $sql .= ' WHERE '.$where.' ORDER BY pagePath=\'/index.php\' DESC, pageSection ASC, pagePath ASC';
            $rows = $this->db->get_rows($sql);
            $pages = $this->return_instances($rows);
            ...
            if (PerchUtil::count($pages)) {
                ...
                foreach($pages as $Page) {
                    if ($i==0||$i==1) {
                        $data = array();
                        $data['pageOrder'] = str_pad($count, 2, '0', STR_PAD_LEFT);
                        $Page->update($data);
                    }else{
                        $sql = 'SELECT * FROM '.$this->table.' WHERE pageDepth='.$this->db->pdb($i-1).' AND pagePath LIKE '.$this->db->pdb($Page->pageSection().'%').' LIMIT 1';
                        $row = $this->db->get_row($sql);
                        if (PerchUtil::count($row)) {
                            $parentOrder = $row['pageOrder'];
                            $sql = 'SELECT COUNT(*) FROM '.$this->table.' WHERE pageOrder LIKE '.$this->db->pdb($parentOrder.'-%').' AND pageOrder NOT LIKE '.$this->db->pdb($parentOrder.'-%-%');
                            $val = $this->db->get_count($sql);
                            $data = array();
                            $data['pageOrder'] = $parentOrder.'-'.str_pad($val+1, 2, '0', STR_PAD_LEFT);
                            ...
                            PerchUtil::debug('No match: '.$Page->pagePath(), 'error');
                            $placeholder = array();
                            $placeholder['pagePath'] = $Page->pageSection().'/index.php';
                            $placeholder['pageDepth'] = $i-1;
                            $placeholder['pageNew'] = 1;
                            $placeholder['navgroupID'] = 0;
                            $placeholder['pagePlaceholder'] = 1;
        ...
        if ($needs_to_rerun) $this->auto_reorder($new_only);
        
    }
    
    private function _page_depth($path) 
    {
        $parts = explode('/', $path);
        $depth = count($parts)-1;

        if ($this->_is_index_page($path)) {
            $depth--;
        }
        return $depth;
    }
    
    private function _page_section($path)
    {
        $parts = explode('/', $path);
    ...
    private function _is_index_page($path)
    {
        $parts = explode('/', $path);
        $file = array_pop($parts);
    ...
    private function get_order_in_section($section)
    {
        ...
        if (PerchUtil::count($row)) {
            ...
            $end++;
            $parts[] = str_pad($end, 2, '0', STR_PAD_LEFT);
