...
class PerchPages_Page extends PerchAPI_Base
{
    ...
    public function delete($kill_file=true)
    {
        if (file_exists(PERCH_SITEPATH.$this->pagePath())) {
            ...
            $this->db->execute($sql);
            if ($kill_file) unlink(PERCH_SITEPATH.$this->pagePath());
        }
    ...
    public function has_more_than_one_child() 
    {
        ...
        if (substr($file, 0, 5)=='index') {
            ...
            $sql = 'SELECT COUNT(*) FROM '.$this->table.' WHERE pageSection LIKE '.$this->db->pdb($path.'%');
